<script lang="ts">
	...
	export let url = 'https://deepsquare.io';
...
{#if open}
	<dialog open transition:fade={{ duration: 50 }}>
		<article>
			<header>
			...
			<main>
				...
				<a
					class="resp-sharing-button__link"
					href="https://www.tumblr.com/widgets/share/tool?posttype=link&amp;title={encodeURIComponent(
						text
					)}&amp;caption={encodeURIComponent(text)}&amp;content={encodeURIComponent(
						url
					)}&amp;canonicalUrl={encodeURIComponent(url)}&amp;shareSource=tumblr_share_button"
					target="_blank"
					rel="noopener"
